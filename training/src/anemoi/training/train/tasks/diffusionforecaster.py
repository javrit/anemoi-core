...
class GraphUnconditionalDiffusionForecaster(GraphDiffusionForecaster):
    ...
    def rollout_step(
        ...
        # print(batch.shape)
        if self.config.training.conditioning.condition=="null":
            x= torch.zeros(
                    (y.shape[0], self.multi_step, *y.shape[1:-1], nvars_input),
                    device=y.device,
                    dtype=y.dtype,
                )
            # print('JE PASSE PAR CONDITION NULLE DANS DIFFUSIONFORECASTER')

        elif self.config.training.conditioning.condition=="orography":
            x = batch[:, 0:self.multi_step, ..., self.config.dataloader.training.select.index('z')]  # shape: [1, 2, 1, 332840]
            x = x.unsqueeze(-1).repeat(1, 1, 1, 1, nvars_input) # shape: [1, 2, 1, 332840, 78]

            # print('JE PASSE DANS CONDITION ORROGRAPHY',nvars_input, self.data_indices.data.input.full,len(self.config.dataloader.training.select),self.config.dataloader.training.select.index('z'))
    ...
    def _get_noise_level(
        ...
        if self.training_approach == "probabilistic_high_noise":
            ...
            LOG.debug(
                "training_approach=probabilistic_low_noise | lognormal_mean=%.3f | lognormal_std=%.3f | shape=%s",
                self.lognormal_mean,
                self.lognormal_std,
                shape,
            )
